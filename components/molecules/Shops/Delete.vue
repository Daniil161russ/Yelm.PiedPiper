<template>
  <el-dialog :close-on-press-escape="false" :visible.sync="DialogDelete" custom-class="Items__AddCategory-Dialog Items__AddCategory-Dialog_EmptyHeader"
    ...
    <div class="Items__AddCategory-Delete" slot="default">
      <div class="Items__AddCategory-Delete_Label">Вы действительно хотите удалить <br>магазин "{{ name }}" ?</div>
...
export default {
  ...
  methods: {
    ...mapActions({
      deleteShop: 'shops/deleteShop'
    }),
    async removeShop () {
      try {
        await this.deleteShop({ id: this.id })
