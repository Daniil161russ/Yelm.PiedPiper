  ...
  <div class="Items__AddCategory">
    <el-dialog :close-on-press-escape="false" :close-on-click-modal="false" :visible.sync="DeleteCategoryDialog"
      ...
      <div class="Items__AddCategory-Delete" slot="default">
        <div class="Items__AddCategory-Delete_Label">Вы действительно хотите удалить <br>категорию "{{ category.name['RU'] }}" ?</div>
...
export default {
  ...
  methods: {
    ...
    async deleteCategory () {
      await this.removeCategory(this.category).then(response => {
        if (response === 403) {
          this.$notify.error({ title: 'Ошибка', message: 'Удалите подкатегории из категории', customClass: 'el-notification-error', duration: 1000  })
        } else if (response == 409) {
          this.$notify.error({ title: 'Ошибка', message: 'Удалите товары из категории', customClass: 'el-notification-error', duration: 1000  })
        }
      })
      this.$emit('hide')
    }
