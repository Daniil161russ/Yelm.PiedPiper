  ...
  <div class="grid__container">
    <div class="grid__body-header">
      <div class="grid__body-header-left">
        ...
        <h2 class="h1">Добавление сотрудника</h2>
      </div>
      <v-button round="" color="primary" plain="" native="button" @click="submitHandler('form')">Добавить</v-button>
    </div>
  ...
  methods: {
    ...
    submitHandler(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$axios.$post('https://delivery.yelm.io/api/signup', this.form).then(() => {
            this.addStaff(this.form)
            this.setSelectedWindow('MapStaff')
          }).catch(error => {
            if (error.response.status === 422) {
              this.$notify.error({ title: 'Ошибка', message: 'Номер телефона уже зарегистрирован!', customClass: 'el-notification-error', duration: 1000  })
            }
          })
        } else {
